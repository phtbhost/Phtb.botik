...
async def cmd_start(message: Message):
    await message.answer(
        "Привет! Я модульный бот для личных чатов Telegram Business. Используй команды:\n"
        "/load <имя_модуля> — загрузить модуль\n"
        "/unload <имя_модуля> — выгрузить модуль\n"
        "/list — список загруженных модулей\n\n"
        "Отправляй сообщения в ЛС бизнес-аккаунта, и я обработаю их с помощью модулей."
    )
...
# Обработка сообщений из бизнес-чата в ЛС
@router.message(lambda message: message.chat.type == "private" and message.business_connection_id is not None)
async def handle_business_message(message: Message):
    await message.answer("Сообщение получено в личном чате бизнес-аккаунта! Модули обрабатывают запрос.")
