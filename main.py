...
async def cmd_start(message: Message):
    await message.answer(
        "Привет! Я модульный бот для всех чатов Telegram Business. Используй команды:\n"
        "/load <имя_модуля> — загрузить модуль\n"
        "/unload <имя_модуля> — выгрузить модуль\n"
        "/list — список загруженных модулей\n\n"
        "Отправляй сообщения в любые чаты бизнес-аккаунта (ЛС, группы, каналы), и я обработаю их с помощью модулей."
    )
...
def load_module_dynamic(module_name):
    try:
        ...
        logging.info(f"Модуль {module_name} загружен")
        return True
...
def unload_module_dynamic(module_name):
    if module_name in active_modules:
        ...
        logging.info(f"Модуль {module_name} выгружен")
        return True
    return False
...
# Обработка сообщений из бизнес-чата (все типы чатов)
@router.message(lambda message: message.business_connection_id is not None)
async def handle_business_message(message: Message):
    chat_type = message.chat.type
    await message.answer(f"Сообщение получено в бизнес-чате (тип: {chat_type})! Модули обрабатывают запрос.")
